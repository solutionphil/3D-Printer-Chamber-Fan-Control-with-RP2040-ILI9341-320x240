...
void loop(void) {
  ...
  } else if (currentScreen == 1) { // PID Control Screen
        if (pressed) {
            upButton.press(pressed && upButton.contains(t_x, t_y));
            downButton.press(pressed && downButton.contains(t_x, t_y));
            toggleButton.press(pressed && toggleButton.contains(t_x, t_y));
            ...
            if (downButton.justPressed()) {
                Setpoint = max(15.0, Setpoint - 0.5);
                savePIDSettings();
                updatePIDDisplay();
            }
            
            if (toggleButton.justPressed()) {
                PIDactive = !PIDactive;
